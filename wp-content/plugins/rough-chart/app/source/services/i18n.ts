import _get from 'lodash/get';

const defaultTranslations = {
    loading: 'Loading...',
    roughCharts: 'Rough Charts',
    addNew: 'Add New',
    created: 'Created',
    edit: 'Edit',
    delete: 'Delete',
    cancel: 'Cancel',
    copy: 'Copy',
    shortcodeCopied: 'Shortcode copied',
    yesDelete: 'Yes, delete',
    noTitle: 'No title',
    deletingChart: '⚠️ Deleting chart',
    areYouSureDelete: 'Are, you sure you want to delete chart?',
    newChart: 'New Chart',
    type: 'Type',
    pie: 'Pie',
    columns: 'Columns',
    bars: 'Bars',
    lines: 'Lines',
    showLegend: 'Show Legend',
    lastUpdated: 'Last updated',
    roughChartsList: 'Rough Charts list',
    newPieChart: 'New Pie Chart',
    editPieChart: 'Edit Pie Chart',
    newBarsChart: 'New Bars Chart',
    editBarsChart: 'Edit Bars Chart',
    newColumnsChart: 'New Columns Chart',
    editColumnsChart: 'Edit Columns Chart',
    newLinesChart: 'New Lines Chart',
    editLinesChart: 'Edit Lines Chart',
    newChartOptions: 'New Rough Chart options',
    createNewChart: 'Create new chart',
    saveChart: 'Save chart',
    title: 'Title',
    fillStyle: 'Fill Style',
    strokeWidth: 'Stroke Width',
    fillColor: 'Fill Color',
    fillWeight: 'Fill Weight',
    fillWeightDescription: 'Weight of inner paths\' color.',
    roughness: 'Roughness',
    roughnessDescription: 'Roughness level of chart.',
    xLabel: 'X Label',
    xLabelDescription: 'Label for x-axis.',
    yLabel: 'Y Label',
    yLabelDescription: 'Label for y-axis.',
    chartData: 'Chart Data',
    noCharts: 'No charts to display (you should create one :)',
    shortcode: 'Shortcode',
    chartAdded: 'Chart added',
    chartSaved: 'Chart saved',
    chartDeleted: 'Chart deleted',
    chartDataAddRowsHint: 'Right click on the table will allow you to add more rows or delete existing ones.',
    defineChart: 'Define your new chart by filling options below:',
    anErrorOccurred: 'An Error occurred',
    goBackToChartsList: 'Go back to charts list',
    valuesShouldBeNumbers: '"Value" column should have numbers only',
    noEmptyCellsInTable: 'Table shouldn\'t have empty cells',
    legendPosition: 'Legend Position',
};

export const t = (translationSlug) => {
    return _get(defaultTranslations, translationSlug, translationSlug);
};
